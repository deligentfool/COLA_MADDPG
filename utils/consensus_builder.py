...
class ConsensusBuilder(nn.Module):
    def __init__(self, obs_dim, consensus_builder_hidden_dim, consensus_builder_dim, ema_tau):
        super(ConsensusBuilder, self).__init__()
        self.obs_dim = obs_dim
        self.ema_tau = ema_tau
        self.consensus_builder_hidden_dim = consensus_builder_hidden_dim
        self.consensus_builder_dim = consensus_builder_dim
    ...
    def update(self):
        for param_o, param_t in zip(self.view_obs_net.parameters(), self.teacher_view_obs_net.parameters()):
            param_t.data = param_t.data * self.ema_tau + param_o.data * (1. - self.ema_tau)
